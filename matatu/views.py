...
import googlemaps
from .models import Stop, Route, FareRule, Booking
from .serializers import StopSerializer, RouteSerializer, FareRuleSerializer, BookingSerializer
...
class BookingViewset(viewsets.ModelViewSet):
    ...
    def start_booking(self, request, pk=None):
        ...
        if not all([stop_id, user_id, route_id]):
            return Response(
                {"error": "Stop ID, user ID, and route ID are required"},
                status=status.HTTP_400_BAD_REQUEST
            )
        
class FareRule(viewsets.ModelViewSet):
    queryset = FareRule.objects.all()
    serializer_class = FareRuleSerializer

    @action(detail=False, methods=['GET'])
    def by_route(self, request, pk=None):
        ...
        try:
            ...
            fare_rules = FareRule.objects.filter(route=route)
            serializer = self.get_serializer(fare_rules, many=True)
            return Response(serializer.data)
